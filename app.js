(() => {
    ...
    const STORAGE = {
        favorites: "sklaqv_favorites_v1",
        history: "sklaqv_history_v1",
        localStatus: "sklaqv_local_status_v1",
        remoteStatus: "sklaqv_remote_status_v1"
    };
    const STATUS = {
        nao_informado: {
    ...
    async function shareCommercialCard() {
        ...
        try {
            ...
            const props = selectedFeature.properties;
            const filename = `Acquaville_Q${String(props.quadra).padStart(2, "0")}_L${String(props.lote).padStart(2, "0")}.png`;
            const destination = props.access ? [ props.access.latitude, props.access.longitude ] : [ props.center[1], props.center[0] ];
            const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${destination[0]},${destination[1]}`;
            const text = `Acquaville Thermas Residence Resort · Quadra ${props.quadra}, Lote ${props.lote} · ${formatNumber(props.area_m2)} m²\nNavegação: ${mapsUrl}\nLocalização para orientação comercial e navegação; não substitui demarcação física ou levantamento técnico.`;
    ...
    async function buildCommercialCard(feature) {
        ...
        context.fillText("LOTEAMENTO", 235, 92);
        context.font = "500 76px Georgia, serif";
        context.fillText("ACQUAVILLE", 230, 160);
        context.font = "700 25px sans-serif";
        context.letterSpacing = "6px";
        context.fillText("THERMAS RESIDENCE RESORT", 235, 205);
